...
unsigned long utf16_to_unicode(char* text){
    ...
    if(high >> 10 == 0x36){
        ...
        unicode = ((high & 0x3ff) << 10) + (low & 0x3ff) + 0x10000;
        sz = 4;
    }
    else{
        unicode = high;
...
size_t strencodencpy(char* src, char* dst, size_t len, size_t max, int encode){
    size_t size = 0;
    size_t idx = 0;
    switch (encode){
    case WORD_ASCII:
...
ssize_t get_utf8_text(struct doc_file* doc, char* text, size_t max){
    ...
    while(secid != END_OF_CHAIN){
        ...
        size += strencodencpy(sector, text + size, doc->sector_size, max - size, encode);
        if(size >= max - 2){
            break;
        }
